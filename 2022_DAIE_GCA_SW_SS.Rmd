...
#H0: there is no significant difference in pre trial OR scores between the 2 genders.
#H1: there is a significant difference between the pre trial OR scores between the 2 genders.
...
INTRODUCTION
The aim of this project is to analyse the impact of Jungian Sandplay therapeutic
technique on individuals with Post-Traumatic Stress Disorder (PTSD). 2 different 
scales were used to assess the patients: 
observer-rated report (OR) and Child PTSD Symptoms Self Report Version (CPSS-SR).
Both use a Likert scale from 0 to 10.

2 sets of hypotheses were tested. 
For the first, one the goal was to verify whether there was any significant 
difference in the pre-trial observed scores between males and female. In other 
words, the researchers wanted to see if gender influenced the pre trial OR scores.
H0: there is no significant difference in pre trial OR scores between the 2 genders.
H1: there is a significant difference in the pre trial OR scores between the 2 genders.

For the second set of hypotheses, the aim was to see if there was any significant
correlation between the pre and post trial OR scores. The pre trial scores 
represented the explanatory variable and the post trial scores the response one.
H0: there is no correlation between pre and post trial OR scores.
H1: there is a significant correlation between pre and post trial scores.

METHOD
150 individuals (75 males and 75 females) were randomly assigned to one of 3 test
groups: Animated, Static and Control.

To start, the relationship between gender and pre trial OR scores was investigated.
Before carrying out an Independent Samples T-Test analysis, the data were cleaned 
and any missing or incorrect values were replaced. A series of descriptive statistics
were also found. Following this, normality tests were applied and non-normal data 
underwent a process of logaritmic normalisation. Once the data were normal, 
the t-test was carried out.

The next step consisted in addressing the second set of hypotheses. The same series
of descriptive statistics were applied, as well as normality tests. After this, 
it was time to test the correlation between pre and post-trial OR scores using
Pearson's correlation.

To add more insights to this analysis, the confidence intervals for the mean of
the pre-trial OR scores, for the difference in means of males and female in pre-trial 
OR scores and for the group mean of pre-trail OR scores for both males and female were also investigated.
